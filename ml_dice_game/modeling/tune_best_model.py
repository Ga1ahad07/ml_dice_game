...
def main(
    ...
    estimator_type = XGBRegressor if selected_name == "XGBoost" else RandomForestRegressor
    params_key = (
        "xgb_param_distributions"
        if selected_name == "XGBoost"
        else "rf_param_distributions"
    )
    estimator = estimator_type(random_state=params["random_state"], n_jobs=-1)
    search = RandomizedSearchCV(
        estimator=estimator,
        param_distributions=params["tune"][params_key],
        n_iter=params["tune"]["n_iter"],
        scoring=params["tune"]["scoring"],
        cv=make_cv(params["split"]["cv_n_splits"], params["random_state"]),
    ...
    comparison = pd.DataFrame({"base": base_metrics, "tuned": tuned_metrics}).T
